...
import { citlaliJadiBot } from './plugins/jadibot-serbot.js';
...
global.authFile = 'CitlaliBotSession'
global.authFileJB  = 'CitlaliJadiBot'
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['CitlaliBot-MD', 'Edge', '20.0.04'] : methodCodeQR ? ['CitlaliBot-MD', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "CitlaliJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}

if (opts['server']) (await import('./server.js')).default(global.conn, PORT)

//respaldo de la sesión "CitlaliBotSession"
...
const sessionDir = './CitlaliBotSession';
try {
...
async function purgeSessionSB() {
  const jadibtsDir = './CitlaliJadiBot/';
  try {
...
const directories = ['./CitlaliBotSession/', './CitlaliJadiBot/'];
for (const dir of directories) {
try {
...
let prekey = []
let directorio = readdirSync("./CitlaliBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./CitlaliBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./CitlaliJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./CitlaliJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./CitlaliJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./CitlaliJadiBot/${directorio}/${fileInDir}`)
}})
}})
if (SBprekey.length === 0) {
console.log(chalk.bold.green(lenguajeGB.smspurgeSessionSB1()))
